...
class MultiObjectGoal(NavigationGoal):
    r"""Multi object goal provides information about an object that is target for
    navigation which is specified by object_category, and position.
    Args:
        object_category: category that can be used to retrieve object from the semantic
        scene annotation
...
class MultiObjectGoalSensor(Sensor):
    ...
    ObjectGoal Navigation. The goal is expected to be specified by object_id.
    Args:
...
class MultiONPPL(Measure):
    r"""PPL (Progress weighted by Path Length)
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        task.measurements.check_measure_dependencies(
            self.uuid, [CurrentGoalSuccess.cls_uuid, Progress.cls_uuid]
        )
        ...
        self._agent_distance_for_subsequent_goals = []
        subsequent_goal_distance = 0
        for goal_number in range(len(episode.goals)):
            if goal_number == 0:
                ...
                self._subsequent_goal_distances.append(
                    self._start_end_episode_distance
                )
            else:
                subsequent_goal_distance += self._sim.geodesic_distance(
    ...
    def update_metric(
        ...
        if ep_current_goal_success:
            self._agent_distance_for_subsequent_goals.append(
        ...
        if ep_progress > 0:
            self._metric = ep_progress * (
                ...
                / max(
                    self._agent_distance_for_subsequent_goals[-1],
                    self._subsequent_goal_distances[
                        task.current_goal_index - 1
                    ],
                )
...
class MultiNavigationTask(NavigationTask):
    ...
    def reset(self, episode: Episode):
        ...
        for i in range(len(episode.goals)):
            ...
            object_index = obj_templates_mgr.load_configs(
                str(os.path.join("data", "multiON_objects", current_goal))
            )[0]
    ...
    def step(self, action: Dict[str, Any], episode: Episode):
        ...
        self.is_found_called = True if action_name == 0 else False
    ...
    def _check_episode_is_active(self, *args: Any, **kwargs: Any) -> bool:
        ...
        distance_to_current_goal = self._sim.geodesic_distance(
            current_position,
            kwargs["episode"].goals[self.current_goal_index].position,
        )
